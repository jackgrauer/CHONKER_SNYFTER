        ...
        Ok(result) => {
            ...
            let chunks_data: Vec<serde_json::Value> = result.chunks.iter().enumerate().map(|(index, chunk)| {
                serde_json::json!({
                    ...
                    "content_preview": chunk.content.clone(),
...
#[tauri::command]
async fn preview_pdf_adjustments(params: PreviewParams) -> Result<String, String> {
    use std::process::Command;
    use uuid::Uuid;
    
    // Generate preview image with adjustments
    let preview_path = format!("/tmp/preview_{}.png", Uuid::new_v4());
    let pdf_page_spec = format!("{}[{}]", params.pdf_path, params.page_num);
    
    let mut args = vec![
        "-density".to_string(), 
        "150".to_string(),  // Lower res for preview
        pdf_page_spec,
    ];
    
    // Apply all transformations
    if let Some(crop) = params.options.crop {
        args.extend(vec![
            "-crop".to_string(),
    ...
    if params.options.deskew_enabled {
        args.extend(vec![
            "-deskew".to_string(),
            format!("{}%", params.options.deskew_threshold)
        ]);
    }
    
    args.extend(vec![
        "-brightness-contrast".to_string(),
        format!("{}x{}", params.options.brightness, params.options.contrast)
    ]);
    
    if params.options.sharpen_enabled {
        args.extend(vec![
            "-sharpen".to_string(),
            format!("0x{}", params.options.sharpen_amount)
        ]);
    }
    
    if params.options.despeckle { args.push("-despeckle".to_string()); }
    if params.options.normalize { args.push("-normalize".to_string()); }
    
    args.push(preview_path.clone());
    
    let output = Command::new("convert").args(&args).output()
        .map_err(|e| format!("Failed to run ImageMagick: {}", e))?;
    
    if !output.status.success() {
        return Err(format!("ImageMagick failed: {}", String::from_utf8_lossy(&output.stderr)));
    }
    
    // Return base64 encoded preview
    let img_data = std::fs::read(&preview_path)
        .map_err(|e| {
            let error_msg = format!("Failed to read preview image: {}", e);
            error_msg
        })?;
    
    // Clean up temp file
    let _ = std::fs::remove_file(6preview_path);
    
    let result = base64::prelude::BASE64_STANDARD.encode(img_data);
    Ok(result)
}

#[tauri::command]
async fn apply_pdf_preprocessing(params: ApplyParams) -> Result<String, String> {
    use std::process::Command;
    
    let output_path = params.pdf_path.replace(".pdf", "_cleaned.pdf");
    
    let mut args = vec![
        "-density".to_string(), 
        "300".to_string(),  // Full resolution for final output
        params.pdf_path.clone(),
    ];

    // Apply all transformations (same as preview but higher quality)
    if let Some(crop) = params.options.crop {
        args.extend(vec![
            "-crop".to_string(),
            format!("{}x{}+{}+{}", crop.width, crop.height, crop.x, crop.y),
...
pub fn run() {
  tauri::Builder::default()
    ...
    .invoke_handler(tauri::generate_handler![
        ...
        update_table_cell,
        preview_pdf_adjustments,
        apply_pdf_preprocessing
    ])
