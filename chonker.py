...
# Removed pdfplumber - wasn't finding tables in these PDFs anyway
...
class DocumentProcessor(QThread):
    ...
    def _extract_with_spatial_layout(self, result, chunks: List[DocumentChunk]) -> str:
        ...
        # Get actual page dimensions from document if available
        page_dimensions = {}
        if hasattr(result.document, 'pages') and result.document.pages:
            for page in result.document.pages:
                if hasattr(page, 'page_no') and hasattr(page, 'width') and hasattr(page, 'height'):
                    page_dimensions[page.page_no] = {
                        'width': page.width,
                        'height': page.height,
                        'rotation': getattr(page, 'rotation', 0)
                    }
                    print(f"🐹 Page {page.page_no}: {page.width}x{page.height}, rotation={getattr(page, 'rotation', 0)}")
        ...
        for page_no in sorted(pages.keys()):
            ...
            # Check if this is a rotated page
            is_rotated = page_no in page_dimensions and page_dimensions[page_no].get('rotation', 0) in [90, 270]
            ...
            for item, level, idx, bbox in pages[page_no]:
                if bbox and hasattr(bbox, 't') and hasattr(bbox, 'r'):
                    ...
                    if hasattr(bbox, 'coord_origin') and str(bbox.coord_origin) == 'CoordOrigin.BOTTOMLEFT':
                        ...
                        max_y = max(max_y, bbox.t + (bbox.b - bbox.t if hasattr(bbox, 'b') else 20))
                    max_x = max(max_x, bbox.r)
                    min_x = min(min_x, bbox.l)
            
            # Get actual page dimensions from PDF metadata
            if page_no in page_dimensions:
                pdf_width = page_dimensions[page_no]['width']
                pdf_height = page_dimensions[page_no]['height']
                rotation = page_dimensions[page_no]['rotation']
                
                # Handle rotation (90 or 270 degrees means landscape)
                if rotation in [90, 270]:
                    # Swap dimensions for landscape
                    page_width = pdf_height * scale
                    page_height = pdf_width * scale
                    print(f"🐹 Page {page_no} is rotated {rotation}° (landscape)")
                else:
                    page_width = pdf_width * scale
                    page_height = pdf_height * scale
            else:
                # Fallback: Calculate from content bounds
                if max_y > 0:
                    # Add some padding
            ...
            # For oversized pages, limit the display width
            display_page_width = page_width
            display_page_height = page_height
            
            if page_width > 1200:
                # Calculate scale to fit width
                width_scale = 1100 / page_width
                display_page_width = 1100
                display_page_height = page_height * width_scale
                print(f"🐹 Limiting page width from {page_width:.0f} to {display_page_width:.0f}")
            
            # Ensure minimum dimensions for very small pages
            display_page_width = max(display_page_width, 600)
            display_page_height = max(display_page_height, 400)
            
            # NOW create the page div with correct dimensions
            # Add class for oversized pages
            page_class = "spatial-page"
            if page_width > 1200 or page_height > 1600:
                page_class += " oversized-page"
                print(f"🐹 Page {page_no} is oversized: {page_width:.0f}x{page_height:.0f}px")
            
            # For oversized pages, add extra container styling
            extra_style = ""
            if page_width > 1200:
                extra_style = " overflow: auto;"
            
            html_parts.append(f'<div class="{page_class}" data-page="{page_no}" '
                            f'style="min-height: {display_page_height}px; width: {display_page_width}px; '
            ...
            for item, level, idx, bbox in pages[page_no]:
                if bbox and hasattr(bbox, 'l') and hasattr(bbox, 't'):
                    ...
                    # For oversized/rotated pages, calculate fit scale FIRST
                    fit_scale = 1.0
                    if page_width > 1200 or is_rotated:
                        # Calculate scale factor to fit content
                        content_width = max_x - min_x
                        content_height = max_y - min_y if max_y > 0 else 792
                        
                        # Calculate appropriate scale to fit wide content
                        if content_width > 0:
                            # Target width is 1000px with some margin
                            target_width = 900
                            fit_scale = min(1.0, target_width / (content_width * scale))
                            
                            # Debug
                            if idx < 3:
                                print(f"  Oversized page: content_width={content_width:.1f}, fit_scale={fit_scale:.2f}")
                    
                    # Apply combined scale
                    combined_scale = scale * fit_scale
                    
                    # Calculate dimensions with combined scale
                    left = bbox.l * combined_scale
                    width = abs(bbox.r - bbox.l) * combined_scale if hasattr(bbox, 'r') else 200
                    height = abs(bbox.b - bbox.t) * combined_scale if hasattr(bbox, 'b') else 20
                    
                    # Add minimum width first
                    width = max(width, 50)
                    
                    # Smart vertical sizing based on content type and length
                    text_content = str(getattr(item, 'text', ''))
                    ...
                    if hasattr(bbox, 'coord_origin') and 'BOTTOMLEFT' in str(bbox.coord_origin):
                        ...
                        top = (max_y - bbox.t) * combined_scale
                        # Also ensure height is calculated correctly
                        if hasattr(bbox, 'b'):
                            actual_height = abs(bbox.t - bbox.b) * combined_scale
                            height = max(actual_height, height)
                    else:
                        top = bbox.t * combined_scale
                    
                    # Also scale font size for oversized pages
                    font_size = 11 * fit_scale if fit_scale < 1.0 else 11
                    
                    # Detect if this looks like a form field first
                    is_form_field = self._is_form_field(item, width, height)
                    
                    # Calculate smart height based on content
                    if item_type in ['Title', 'SectionHeader', 'SectionHeaderItem']:
                        # Headers: compact height
                        height = max(font_size * 1.4, 20)
                    elif item_type in ['Paragraph', 'ParagraphItem']:
                        # Paragraphs: calculate based on estimated wrapped lines
                        if width > 100:
                            estimated_chars_per_line = width / (font_size * 0.6)
                            estimated_lines = max(1, text_length / estimated_chars_per_line)
                            height = max(estimated_lines * font_size * 1.2, 20)
                        else:
                            height = max(height, 20)
                    elif is_form_field:
                        # Form fields: minimal height
                        height = max(font_size * 1.5, 18)
                    elif text_length < 30:
                        # Short text: compact height
                        height = max(font_size * 1.3, 18)
                    else:
                    ...
                    # Add z-index based on vertical position to prevent overlap issues
                    z_index = int(top) if not is_form_field else int(top) + 1000
                    
                    # Smart text wrapping decision
                    text_content = str(getattr(item, 'text', ''))
                    text_length = len(text_content)
                    avg_char_width = font_size * 0.6  # Approximate character width
                    expected_text_width = text_length * avg_char_width
                    item_type = type(item).__name__
                    ...
                    if width < 100 or is_form_field:
                        ...
                        # Paragraphs with reasonable width: always wrap
                        white_space_style = "white-space: normal; word-wrap: break-word;"
                        # Also increase min-height for paragraphs
                        if text_length > 100:
                            height = max(height, (text_length / (width / avg_char_width)) * font_size * 1.2)
                    elif text_length < 20:
                        # Short text: don't wrap
                        white_space_style = "white-space: nowrap;"
                    elif expected_text_width > width * 1.5 and width > 200:
                        # Long text in reasonably wide container: allow wrapping
                        white_space_style = "white-space: normal; word-wrap: break-word;"
                    elif item_type in ['Title', 'SectionHeader', 'SectionHeaderItem']:
                        # Headers: wrap only if very long
                        if text_length > 40 and width > 200:
                            white_space_style = "white-space: normal; word-wrap: break-word;"
                        else:
                            white_space_style = "white-space: nowrap;"
                    elif ":" in text_content or "?" in text_content:
                        # Labels and questions: prefer no wrap unless very long
                        if text_length > 50 and width > 300:
                            white_space_style = "white-space: normal; word-wrap: break-word;"
                        else:
                            white_space_style = "white-space: nowrap;"
                    elif width < expected_text_width:
                        # Text wider than container: wrap if container is reasonably wide
                        if width > 150:
                            white_space_style = "white-space: normal; word-wrap: break-word;"
                        else:
                            white_space_style = "white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
                    else:
                        # Default: no wrap for clean layout
                        white_space_style = "white-space: nowrap;"
                    
                    html_parts.append(
                        f'<div class="spatial-item{" form-field" if is_form_field else ""}" '
                        f'style="left: {left}px; top: {top}px; width: {width}px; min-height: {height}px; font-size: {font_size}px; z-index: {z_index}; {white_space_style}"'
                        f'{debug_info}>'
                    )
                    # Add content in a span that's editable
                    html_parts.append('<span contenteditable="true" style="user-select: text; outline: none;">')
            ...
            # Removed pdfplumber scan - wasn't finding tables in these PDFs
    ...
    def _extract_linear(self, result, chunks: List[DocumentChunk]) -> str:
        ...
        for idx, (item, level) in enumerate(items):
            ...
            # Update page number without adding visual indicator
    ...
    def _table_to_html(self, table_item) -> str:
        html = ['<table class="editable-table" border="1" style="table-layout: auto; width: auto; max-width: 100%;">']
            ...
            try:
                ...
                if text and text.strip() and not text.startswith('table_cells=') and '|' in text:
                    ...
                    for line in lines:
                        ...
                        for cell in cells:
                            if cell.strip():
                                cell_content = escape(cell.strip())
                                # Smart wrapping for table cells
                                cell_style = ""
                                if len(cell_content) > 50:
                                    # Long content: set max width and wrap
                                    cell_style = ' style="max-width: 300px; white-space: normal; word-wrap: break-word;"'
                                elif len(cell_content) > 30:
                                    # Medium content: set reasonable width
                                    cell_style = ' style="max-width: 200px; white-space: normal;"'
                                html.append(f'<td contenteditable="true"{cell_style}>{cell_content}</td>')
                        html.append('</tr>')
                ...
                for _, row in df.iterrows():
                    ...
                    for value in row:
                        ...
                        if len(safe_value) > 50:
                            # Long content: set max width and wrap
                            cell_style = ' style="max-width: 300px; white-space: normal; word-wrap: break-word;"'
                        elif len(safe_value) > 30:
                            # Medium content: set reasonable width
                            cell_style = ' style="max-width: 200px; white-space: normal;"'
                        html.append(f'<td contenteditable="true"{cell_style}>{safe_value}</td>')
                    html.append('</tr>')
...
class ChonkerApp(QMainWindow):
    ...
    def _init_ui(self):
        ...
        if WEBENGINE_AVAILABLE:
            ...
            <head>
                <style>
                    body { 
                        ...
                        padding: 10px;
                        font-size: 14px;
                        overflow-x: auto;  /* Allow horizontal scroll for oversized pages */
                    }
                </style>
            </head>
            <body>
            ...
            self.faithful_output.setStyleSheet("QTextEdit {background-color: #525659; color: #FFFFFF; border: none;}")
    ...
    def process_multiple_files(self, pdf_files):
        ...
        for idx, pdf_path in enumerate(pdf_files):
            ...
            try:
                ...
                # Add content directly without headers
                all_html.append(html_content)
                # Simple separator between files
                if idx < len(pdf_files) - 1:
                    all_html.append('<hr style="border: 1px solid #525659; margin: 20px 0;">')
    ...
    def _display_in_faithful_output(self, result: ProcessingResult):
        ...
        html_parts.append(
            ...
            f'body{{font-family:Arial,Helvetica,sans-serif;margin:0;padding:10px;color:#FFF;background:#525659;font-size:{zoom_size}px!important}}'
            f'#document-content{{border:none!important;outline:none!important;padding:0;}}'
            ...
            f'th,td{{border:1px solid #525659;padding:4px 6px;color:#FFF;background:#424548;font-size:{zoom_size}px!important}}'
            ...
            f'.spatial-page {{ position: relative; width: 100%; min-height: 1000px; margin-bottom: 20px; border: none; background: transparent; overflow: hidden; }}'
            f'.oversized-page {{ border: none; background: transparent; overflow: auto !important; }}'
            f'.oversized-page::before {{ content: "⚠️ Oversized/Rotated Page"; position: absolute; top: -25px; left: 10px; color: #E67E22; font-size: 12px; background: #525659; padding: 2px 8px; z-index: 9999; }}'
            f'.spatial-item {{ position: absolute !important; border: 1px solid transparent; padding: 5px 8px; padding-left: 20px; color: #FFF; background: transparent; font-size: {zoom_size * 0.5}px !important; line-height: 1.1; overflow: visible; transition: all 0.2s ease; }}'
            f'.spatial-item::before {{ content: "⋮⋮"; position: absolute; left: 2px; top: 50%; transform: translateY(-50%); color: #666; font-size: 10px; opacity: 0; transition: opacity 0.2s; }}'
            f'.spatial-item:hover {{ color: #1ABC9C; background: rgba(26, 188, 156, 0.1); border: 1px solid rgba(26, 188, 156, 0.3); z-index: 999999 !important; cursor: move; }}'
            f'.spatial-item:hover::before {{ opacity: 1; color: #1ABC9C; }}'
            f'.spatial-item span[contenteditable]:hover {{ cursor: text !important; }}'
            f'.spatial-item span[contenteditable]:focus {{ background: rgba(26, 188, 156, 0.2); padding: 2px; border-radius: 2px; }}'
            f'.form-field {{ background: transparent !important; border: none !important; color: #FFF; }}'
            f'.spatial-item.dragging {{ opacity: 0.5; cursor: grabbing !important; }}'
            f'</style>'
        )
        ...
        html_parts.append(
            ...
            '    const item = e.target.closest(".spatial-item");'
            '    if (!item) return;'
            '    '
            ...
            '    startY = e.clientY;'
            '    '
            '    // Check if clicking on contenteditable span'
            '    const span = e.target.closest("span[contenteditable]");'
            '    if (span) {'
            '      // Allow text selection, but prepare for possible drag'
            ...
            '          // User is dragging, not selecting text (reduced from 10px to 3px)'
            '          isDragging = true;'
            '          e.preventDefault();'
            ...
            '      setTimeout(() => document.removeEventListener("mousemove", handleMove), 100);'
            '    } else {'
            '      // Clicking on item background - immediate drag'
            '      e.preventDefault();'
            '      isDragging = true;'
            '      draggedElement = item;'
            '      item.classList.add("dragging");'
            ...
            '    e.preventDefault();'
            '    const parent = draggedElement.closest(".spatial-page");'
            '    if (!parent) return;'
            ...
            '      draggedElement.setAttribute("data-dragged", "true");'
            '      draggedElement = null;'
            '    }'
            '    isDragging = false;'
            '  });'
            ...
            '  style.textContent = ".spatial-item { position: relative; } ";'
            '  style.textContent += ".spatial-item::before { content: \'\'; position: absolute; ";'
            '  style.textContent += "top: -5px; left: -5px; right: -5px; bottom: -5px; z-index: -1; }";'
    ...
    def create_output_window(self, result: ProcessingResult):
        ...
        html = (
            f'<!DOCTYPE html><html><head><style>'
            f'body {{ font-family: Arial, Helvetica, sans-serif; margin: 0; padding: 10px; color: #FFFFFF; background: #525659; }}'
            f'table {{ border-collapse: collapse; margin: 15px 0; border: 1px solid #888888; table-layout: auto; max-width: 100%; }}'
            f'th, td {{ border: 1px solid #888888; padding: 4px 6px; color: #000000; }}'
