...
class ChonkerApp(QMainWindow):
    ...
    def export_to_arrow(self):
        """Export document to partitioned Arrow dataset for efficient querying"""
        if not hasattr(self, '_last_processing_result') or not self._last_processing_result:
            QMessageBox.warning(self, "No Document", "Please process a document first")
            return
        ...
        if not PYARROW_AVAILABLE:
            QMessageBox.critical(self, "PyArrow Not Available", 
        ...
        export_dir = QFileDialog.getExistingDirectory(
            self,
            "Select Arrow Dataset Directory",
            "",
        ...
        source_name = os.path.basename(self.current_pdf_path).replace('.pdf', '') if hasattr(self, 'current_pdf_path') else "unknown"
        dataset_name = f"chonker_arrow_{source_name}_{timestamp}"
        dataset_path = os.path.join(export_dir, dataset_name)
        ...
        # Show progress
        progress = QProgressDialog("Exporting to Arrow dataset...", None, 0, 100, self)
        progress.setWindowModality(Qt.WindowModality.WindowModal)
        progress.setMinimumDuration(0)
        ...
        try:
            ...
            exporter = ArrowDatasetExporter(dataset_path)
            
            # Generate document ID
            doc_id = f"doc_{timestamp}_{hashlib.md5(source_name.encode()).hexdigest()[:8]}"
            
            progress.setValue(50)
            
            # Export the document
            result_msg = exporter.export_document_dataset(
                doc_id=doc_id,
                html_content=content_html,
                source_pdf=self.current_pdf_path if hasattr(self, 'current_pdf_path') else None
            )
...
"""
Example queries for the exported Arrow dataset
...
# Example 1: Get all headers
headers = dataset.to_table(
    filter=(ds.field("semantic_role") == "header")
).to_pandas()
print(f"Found {{len(headers)}} headers")
print(headers[['plain_text', 'element_type']].head())

# Example 2: Find financial text
financial = dataset.to_table(
    filter=(ds.field("semantic_role") == "financial_text")
).to_pandas()
print(f"\\nFound {{len(financial)}} financial elements")

# Example 3: Find bold text containing specific keywords
bold_revenue = dataset.to_table(
    filter=(
        (ds.field("style_bold") == True) & 
        pc.match_substring(ds.field("plain_text"), "revenue", ignore_case=True)
    )
).to_pandas()
print(f"\\nFound {{len(bold_revenue)}} bold elements mentioning 'revenue'")

# Example 4: Get all tables
tables = dataset.to_table(
    filter=(ds.field("element_type") == "table")
).to_pandas()
print(f"\\nFound {{len(tables)}} tables")

# Example 5: Complex query - Headers followed by financial text
# This would require joining on element_order, left as exercise
'''
            
            example_path = os.path.join(dataset_path, "example_queries.py")
            ...
            # Show success message
            summary = f"Arrow Dataset Export Complete!\n\n"
            summary += f"Dataset Path: {dataset_path}\n"
            summary += f"Document ID: {doc_id}\n\n"
            summary += result_msg + "\n\n"
            summary += "Features:\n"
            summary += "  • Partitioned by document and element type\n"
            summary += "  • Rich metadata including styles and semantic roles\n"
            summary += "  • Efficient columnar storage\n"
            summary += "  • Example queries script included\n\n"
            summary += "Query with: ds.dataset(path).to_table(filter=...)"
            
            msg_box = QMessageBox(self)
            msg_box.setIcon(QMessageBox.Icon.Information)
            msg_box.setWindowTitle("Export Successful")
            msg_box.setText(summary)
            msg_box.setDetailedText(f"Example query script created at:\n{example_path}")
            msg_box.exec()
            
            self.log(f"Exported Arrow dataset to: {dataset_path}")
            
        except Exception as e:
            progress.close()
            QMessageBox.critical(self, "Export Failed", f"Failed to export Arrow dataset:\n\n{str(e)}")
            self.log(f"Arrow export failed: {str(e)}")
    ...
    def _create_menu_bar(self):
        ...
        export_arrow_action = QAction("Export to Arrow Dataset", self)
        export_arrow_action.setShortcut(QKeySequence("Ctrl+Shift+E"))
        export_arrow_action.triggered.connect(self.export_to_arrow)
        file_menu.addAction(export_arrow_action)
