...
impl PdfCache {
    ...
    pub fn change_page(&mut self, new_page: usize, total_pages: usize) -> Result<Option<String>> {
        ...
        let cached_data = if new_page == self.current.as_ref().map(|d| d.page_num + 1).unwrap_or(usize::MAX) {
            self.next.take()
        } else if new_page == self.current.as_ref().map(|d| d.page_num.saturating_sub(1)).unwrap_or(usize::MAX) {
            self.prev.take()
        } else {
            // Check LRU cache
    ...
    fn prerender_adjacent_pages(&mut self, current_page: usize, total_pages: usize) {
        ...
        let need_prev = current_page > 0 && self.prev.as_ref().map(|d| d.page_num) != Some(current_page - 1);
        let need_next = current_page + 1 < total_pages && self.next.as_ref().map(|d| d.page_num) != Some(current_page + 1);
        ...
        if Command::new("mutool").arg("--version").output().is_ok() {
            ...
            let output = Command::new("mutool")
                .args([
                    "draw",
                    "-F", "txt",  // Text output
                    "-r", &dpi.to_string(), // Optimal DPI
                    "-c", "gray", // Grayscale for speed
                    "-o", "-",    // Output to stdout
