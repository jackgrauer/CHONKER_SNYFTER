...
class AnimatedLabel(QLabel):
    ...
    def extract_html_content(self, docling_result, all_items) -> str:
        """Extract HTML using the proven method from Qt Mixed version"""
        try:
            html_parts = []
            html_parts.append('<div id="document-content" contenteditable="true">')
            
            table_id = 0
            
            self.progress.emit("🐹 Building HTML content...")
            ...
            for item, level in all_items:
                ...
                items_processed += 1
                item_types_found[item_type] = item_types_found.get(item_type, 0) + 1
                
                if item_type == 'SectionHeaderItem' and hasattr(item, 'text'):
                    heading_level = min(level + 1, 3)  # level starts at 0
                    text = str(item.text).strip()
                    if text:
                        html_parts.append(f'<h{heading_level}>{text}</h{heading_level}>')
                        print(f"🐹 Added heading: {text[:50]}...")
                    
                elif item_type == 'TableItem':
                    table_id += 1
                    # Create a simple HTML table with editable class
                    html_parts.append('<table class="extracted-table editable-table" border="1">')
                        ...
                        try:
                            ...
                            if not df.empty:
                                ...
                                for _, row in df.iterrows():
                                    ...
                                    html_parts.append('</tr>')
                                table_added = True
                                print(f"🐹 Added table with {len(df)} rows")
                        except Exception as e:
                            print(f"🐹 Table export_to_dataframe failed: {e}")
                    
                    # Method 2: Try direct row access
                    if not table_added and hasattr(item, 'rows'):
                    ...
                    # Method 3: Try to get any text representation
                    if not table_added:
                        if hasattr(item, 'text'):
                            html_parts.append(f'<tr><td>{item.text}</td></tr>')
                        else:
                            # Last resort - just indicate table exists
                            html_parts.append('<tr><td>[Table]</td></tr>')
            ...
            print(f"🐹 Processed {items_processed} items, generated {len(html_parts)} HTML parts")
            print(f"🐹 Item types found: {item_types_found}")
            
            # If no items found, try markdown export as fallback
            if len(html_parts) <= 2:
                try:
            ...
            self.progress.emit(f"🐹 *cough* HTML extraction error: {str(e)}")
            # Return simple fallback HTML
            return '<div id="document-content"><p>Error extracting HTML content</p></div>'
    ...
    def run(self):
        ...
        try:
            ...
            # Add debug logging
            print(f"🐹 Starting processing of: {self.pdf_path}")
            ...
            # Import docling here to ensure it's available
            try:
                from docling.document_converter import DocumentConverter
                
                # Workaround for tqdm issue
                import tqdm
                ...
                error_msg = f"🐹 *cough* Can't find docling! Make sure it's installed: {str(e)}"
                print(error_msg)
            ...
            try:
                ...
                error_msg = f"Failed to convert PDF: {str(conv_error)}"
            ...
            print("🐹 Collecting document items...")
            ...
            # Extract HTML using the proven method
            print("🐹 Extracting HTML content...")
            try:
                html_content = self.extract_html_content(result, all_items)
                print(f"🐹 HTML extracted: {len(html_content)} characters")
            except Exception as html_error:
                print(f"🐹 HTML extraction failed: {str(html_error)}")
                import traceback
                traceback.print_exc()
                html_content = "<div>Error extracting HTML</div>"
            ...
            print(f"🐹 SUCCESS! Processing complete in {processing_time:.2f} seconds")
            print(f"🐹 Emitting finished signal with {len(chunks)} chunks")
...
class ChonkerSnyfterEnhancedWindow(QMainWindow):
    ...
    def update_terminal(self, message: str, level: str = "info"):
    ...
    def create_pdf_window(self, file_path: str):
        ...
        # PDF viewer
        pdf_view = QPdfView(pdf_window)
        pdf_document = QPdfDocument(pdf_window)
    ...
    def process_current_pdf(self):
        ...
        self.update_terminal("🐹 *munch munch* Starting to process...")
    ...
    def on_processing_progress(self, message: str):
        ...
        # Update terminal instead of status label
    ...
    def on_chunk_progress(self, current: int, total: int):
        ...
        if total > 0:
            ...
            progress_msg = f"📊 Progress: {current}/{total} chunks"
            self.update_terminal(progress_msg)
    ...
    def on_processing_finished(self, result: ProcessingResult):
        ...
        if result.success:
            ...
            if self.snyfter_db.save_document(result, self.current_pdf_path):
                ...
                self.update_terminal(success_msg)
                ...
                self.update_terminal(warning_msg)
    ...
    def display_processing_results(self, result: ProcessingResult):
        ...
        print(f"🐹 First 500 chars of HTML: {result.html_content[:500]}")
        ...
        try:
            ...
            <head>
                ...
                <style>
                    ...
                    table.editable-table td[contenteditable="true"]:hover {{
                        background-color: #f0f0f0;
                        cursor: text;
                    }}
                    table.editable-table td[contenteditable="true"]:focus {{
            ...
            html_output += """
                ...
                function makeTablesEditable() {
                    ...
                    tables.forEach((table, tableIndex) => {
                        ...
                        table.setAttribute('data-table-id', tableIndex);
                        ...
                        rows.forEach((row, rowIndex) => {
                            if (rowIndex > 0 || !row.querySelector('th')) { // Skip header row
                        ...
                        addColBtn.onclick = function() {
                            ...
                            for (let i = 0; i < rows.length; i++) {
                                ...
                                if (i === 0 && rows[i].cells[0].tagName === 'TH') {
                                    // Convert to TH for header row
                                    const th = document.createElement('th');
                                    th.innerHTML = 'New Column';
                                    th.setAttribute('contenteditable', 'true');
                                    rows[i].replaceChild(th, newCell);
                                } else {
                                    newCell.setAttribute('contenteditable', 'true');
                                    newCell.innerHTML = '&nbsp;';
                                }
                ...
                document.addEventListener('DOMContentLoaded', makeTablesEditable);
                makeTablesEditable(); // Also run immediately
                </script>
            </body>
            </html>
            """
    ...
    def create_output_window(self, result: ProcessingResult):
        ...
        if hasattr(self, 'faithful_output') and self.faithful_output.toHtml():
            ...
            try:
                ...
                <head>
                    <style>
                        body {{ font-family: -apple-system, sans-serif; margin: 20px; color: #000000; }}
                        table {{ border-collapse: collapse; width: 100%; margin: 15px 0; border: 1px solid #888888; }}
                        th, td {{ border: 1px solid #888888; padding: 8px; text-align: left; color: #000000; }}
                        th {{ background-color: #f5f5f5; color: #000000; }}
    ...
    def clean_current_pdf(self):
        ...
        try:
            ...
            self.update_terminal("✅ PDF cleaned successfully!")
    ...
    def search_archives(self):
        ...
        self.update_terminal(f"Found {len(results)} documents")
    ...
    def update_archive_stats(self):
        ...
        try:
            ...
            # Update terminal with stats instead of non-existent labels
            self.update_terminal(f"📊 Archive: {total_docs} documents, {total_chunks} chunks (Last: {last_date})")
            
        except Exception as e:
            self.update_terminal(f"🐁 Error updating statistics: {str(e)}")
    ...
    def new_session(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            # Close any floating PDF windows
            if hasattr(self, 'floating_panels'):
                for panel_id in list(self.floating_panels.keys()):
                    if panel_id.startswith('pdf_'):
                        self.floating_panels[panel_id].close()
                        del self.floating_panels[panel_id]
            
            self.update_terminal("🆕 New session started - Ready for new documents!")
...
def handle_exception(exc_type, exc_value, exc_traceback):
    ...
    print("🐹 UNCAUGHT EXCEPTION - App may crash!")
